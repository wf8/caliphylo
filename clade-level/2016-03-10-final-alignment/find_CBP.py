...
seq_files = ["Californian master sequences/California_18S_master.fasta",
         ...
         "Californian master sequences/California_matR_master.fasta",
         ...
         "Californian master sequences/California_rps4.fasta"]
...
def get_accession(description):
    ...
    if "_CBP" in description:
        ...
        if "subsp." in words[2] or "var." in words[2]:
            return "_".join( words[0:5] )
        else:
            return "_".join( words[0:3] )
...
csv_rows = []
csv_rows.append(["clade", "18S", "ITS", "atpB", "matK", "matR", "ndhF", "rbcL", "trnL_trnF", "rps4"])
for clade in clades.iterkeys():
    row = []
    row.append(clade)
    found = False
...
with open("accessions_CBP.csv", "wb") as csvfile:
