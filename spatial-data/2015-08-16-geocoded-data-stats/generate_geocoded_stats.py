...
org_files = ["output/0.txt","output/1.txt",
            "output/2.txt","output/3.txt"]
...
for i, rfile in enumerate(org_files):
    with open(rfile, 'rb') as csvfile:
        csvreader = csv.reader(csvfile, delimiter="\t", quoting=csv.QUOTE_NONE)
        for row in csvreader:
            if row[9] not in all_counties:
                all_counties.append(row[9])
            clades[row[21]].update(row)
            if row[21] != "?":
