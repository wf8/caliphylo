...
org_files = ["input/CCH_clades_by_counties_Part_11.csv"]
...
with open(org_files[0], 'rb') as csvfile:
    ...
    for row in csvreader:
        if num_records != 0:
            if row[lon] != '' and row[lat] != '' and row[locality] != '' and row[collector] != '' and row[date] != '':
...
with open(org_files[0], 'rb') as csvfile:
    ...
    for row in csvreader:
        if row_num != 0:
            ...
            if row[lon] == '' and row[lat] == '' and row[locality] != '' and row[collector] != '' and row[date] != '':
                ...
                for record in complete_records:
                    try:
                        if int(record.year) == int(row_year) and int(record.month) == int(row_month) and int(record.day) == int(row_day) and record.collector == row[collector] and record.locality == row[locality]:
                            cloned_records += 1
                            row[lat] = record.lat
                            row[lon] = record.lon
                            cloned = True
                        ...
                        try:
                            if int(record.year) == int(row_year) and int(record.month) == int(row_month) and record.collector == row[collector] and record.locality == row[locality]:
                                cloned_records += 1
                                row[lat] = record.lat
                                row[lon] = record.lon
                                cloned = True
                            ...
                            try:
                                if int(record.year) == int(row_year) and record.collector == row[collector] and record.locality == row[locality]:
                                    cloned_records += 1
                                    row[lat] = record.lat
                                    row[lon] = record.lon
                                    cloned = True
        ...
        row_num += 1
        with open("output/CCH_clades_by_counties_cloned-with-collector_Part_11.txt", "a") as outputfile:
